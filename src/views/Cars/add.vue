    ...
    <div class="parking-add">
        <VueForm ref="vueForm" :formData="form_data" :formItme="form_item" :formHandler="form_handler">
            <template v-slot:maintain>
                <el-row :gutter="30">
                    <el-col :span="6">
                        <el-date-picker v-model="form_data.maintainDate" vaule-format="yyyy-MM-dd" type="date" placeholder="选择日期" style="width:100%"></el-date-picker>
                    </el-col>
                   
                </el-row>
            </template>
            <template v-slot:maintainNext>
                <el-row :gutter="30">
                    
                    <el-col :span="6">
                         <el-date-picker v-model="form_data.maintainNextDate" vaule-format="yyyy-MM-dd" type="date" placeholder="选择日期" style="width:100%"></el-date-picker>
                    </el-col>
                </el-row>
            </template>
            <template v-slot:energy>
                ...
                <div class="progerss-bar-wrap" v-if="form_data.energyType == 3 || form_data.energyType == 1">
                ...
                <div class="progerss-bar-wrap" v-if="form_data.energyType == 3 || form_data.energyType == 2">
...
import { CarAdd,CarsDetailed,CarsEdit } from "@/api/cars";
export default {
    name:"CarsAdd",
    components:{ VueForm },
    data(){
        return{
            ...
            form_item:[
                ...
                {
                    type:"Slot",
                    slotName:"maintainNext",
                    prop:"maintainNextDate",
                    label:" 下次保养日期"
                },
                {
                    type:"Radio",
    ...
    beforeMount(){
        ...
        console.log(this.$store.state.config.energyType)
    ...
    methods:{
        ...
        add(){
            ...
            CarAdd(this.form_data).then(response=>{
                this.$message({
                    type:"success",
                    message:response.message
                })
        ...
        /**删除车辆属性 */
        delCarsAttr(index){
            this.cars_attr.splice(index,1);//第一个参数：指定数组索引的位置，第二个参数从指定索引位置删除多少个，删除的个数
        },
        /**车辆属性格式化 */
        formatCarsAttr(){
